...
func _ready():
	for c in 6:
		for r in 12:
			...
			sprite.texture = preload('res://textures/player_fragment.png')
			sprite.position.x = c * 16 - 40
			sprite.position.y = r * 16 - 88
			
			add_child(sprite)

func animate(speed, on_finished):
	...
	for child in get_children():
		...
		var offset_h = (child.position - closest_point_h) * sin((1 - closest_point_h.length() / HALF_DIAGONAL_LENGTH) * PI / 2) * speed
	...
	tween.tween_property(self, 'modulate', YELLOW, 0.65).from(RED)
	tween.parallel()
